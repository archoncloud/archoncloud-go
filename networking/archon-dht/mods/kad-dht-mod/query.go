...
func (r *dhtQueryRunner) queryPeer(proc process.Process, p peer.ID) {
	...
	if err != nil {
		...
		if r.query.dht.permissionLayer.ID() != "NON" {
			to := time.Duration(2000) * time.Millisecond
			validated, _ := r.query.dht.permissionLayer.ValidatePeersPtrArr(res.closerPeers, to) // w timeout
			res.closerPeers = nil
			res.closerPeers = append(res.closerPeers, validated...)
		}
